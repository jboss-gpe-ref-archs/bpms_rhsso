...
----
+
This creates a user `admin1` with password `admin`. Save the file.
+
...
The `angularjs-client` folder in this repository contains the code for a KIE Server client. +
The client application runs on Node.js, and uses a AngularJS frontend. The Node.js application serves as a proxy for the KIE server. +
...
Open the file `angularjs-client/config.js`. +
...
$ node server.js
----
. This starts the Nodejs server, bound to `localhost:8000`. Open a browser window and navigate to `http://localhost:8080`. +
...
If you are interested how all this works on the Node and AngularJS side, you should have a look at the code.
...
All this happens in the `public/scripts/app.js` script, in the `initializeKeyCloak` function
...
On the server side, a number of routes are exposed using the Express framework. These routes act as proxies for the REST API of KIE server. These routes are defined in the `app/routes` folder.


Every time a request to the Node server is made, the token is set in the `Authorization` header of the request. On the Node server side, the token value is extracted from the request, and inserted in the request to the KIE Server.+
For example, to get the list of tasks assigned to the logged in user (in `public/scripts/controllers/mytickets.js`):
...
On the server side, the route implementation of the `/api/mytickets` endpoint builds a request to the KIE Server, and sends the reply back to the client.
